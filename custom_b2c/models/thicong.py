...
class ThiCong(models.Model):
    ...
    ngay_batdau = fields.Date(string='Ngày bắt đầu', required=True, track_visibility='onchange',
    ...
    nhansu_thicong = fields.One2many(comodel_name='nhansu.thicong', inverse_name='cong_trinh', required=True,
                                     string='Nhân sự thi công')
    ...
    ctag_thicong = fields.Selection([('1', 'Danh mục 1...'), ('2', 'Danh mục 2...'), ('3', 'Danh mục 3...')],
                                    string='Danh mục')
    ...
    def depend_location(self):
        self.stock_picking_type = self.env['stock.picking.type'].search(
            ['&', ('code', '=', 'outgoing'), ('default_location_src_id', '=', self.location_id.id)], limit=1)
...
class NhanSuThiCong(models.Model):
    ...
    def get_luong(self):
        for rec in self:
            hop_dong = rec.env['hr.contract'].search(
                ['&', ('employee_id', '=', rec.nhan_vien.id), ('state', '=', 'open')], limit=1)
